...
router.put('/change-password', checkToken, async (req, res) => {
  try {
    ...
    if (newPassword.length < 6) {
      return res.status(400).json({ result: false, message: 'Le nouveau mot de passe doit contenir au moins 6 caractères.' });
    }
...
// --------- 3) Demander un reset par OTP (mobile) ----------
...
router.post('/forgot-password-otp', forgotLimiter, async (req, res) => {
  ...
  try {
    ...
    const generic = { result: true, message: 'Si un compte existe, un code a été envoyé.' };
    if (!user) return res.json(generic);

    // Invalider les anciens codes reset non utilisés
    await EmailOtp.updateMany(
      { userId: user._id, purpose: 'reset_password', usedAt: null },
      { $set: { usedAt: new Date() } }
    );
    ...
    await sendPasswordResetOtp({ toEmail: user.email, toName: user.username || '', code });
    return res.json(generic);
  } catch (e) {
    console.error('[FORGOT-OTP]', e);
    res.status(500).json({ result: false, error: 'Server error' });
  }
});

// --------- 4) Confirmer le reset par OTP (mobile) ----------
// Vérifie le code, change le mot de passe, invalide toutes les sessions.
router.post('/reset-password-otp', async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(400).json({ result: false, error: 'Code invalide ou expiré' });
    ...
    if (!rec) return res.status(400).json({ result: false, error: 'Code invalide ou expiré' });

    // Bloquer après 5 mauvais essais
    if (rec.attempts >= 5) return res.status(429).json({ result: false, error: 'Trop de tentatives' });

    const ok = await compare(code, rec.codeHash);
    if (!ok) {
      rec.attempts += 1;
      await rec.save();
      return res.status(400).json({ result: false, error: 'Code invalide ou expiré' });
    }
    ...
    // Invalider le code et toutes les sessions actives
    rec.usedAt = new Date();
    await rec.save();
    await Session.deleteMany({ userId: user._id });

    return res.json({ result: true, message: 'Mot de passe réinitialisé.' });
  } catch (e) {
    console.error('[RESET-OTP]', e);
    res.status(500).json({ result: false, error: 'Server error' });
  }
});
